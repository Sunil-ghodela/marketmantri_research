...
| 10 | **Sharpe under K=15 — pending** | The same basket strategy, data, costs and sizing as #2, re-run with the live engine's concurrency cap: a chronological walk admitting a trade only if fewer than 15 positions are open, ties broken in `WATCH` order, one position per stock, portfolio-level loss-cluster sizing. The apples-to-apples number for the live configuration | run started 2026-07-28 | `_basket_k15_backtest.py` (private) → `basket_k15_report.md` | ⏳ **running.** The same run also emits the unconstrained (K=46) figure on identical data, so the cap's cost is measured within one run rather than against #2, whose data ended 22 Jul |
